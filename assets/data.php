<?php
 return [
    'about' => [
        'name' => 'Василий Шитухин',
        'post' => 'Full Stack Developer',
        'email' => 'thonny02@mail.ru',
        'phone' => '+79888902502',
        'site' => 'github',
        'github' => 'github.com/thonny'
    ],
    'education' => [
        [
            'faculty' => 'Информационная безопасность автоматизированных систем(очно)',
            'univercity' => 'ТМехК',
            'yearStart' => 2018,
            'yearEnd' => 2022
        ],
        [
            'faculty' => 'Программирование в компьютерных системах(заочно)',
            'univercity' => 'ТМехК',
            'yearStart' => 2018,
            'yearEnd' => 2022
        ]
    ],
    'languages' => [
        'English', 'Russian', 'Spanish'
    ],
    'interests' => [
        'чтение книг',
        'компьютерные игры'
    ],
    'aboutCareer' => 
        'Summarise your career here lorem ipsum dolor sit amet, consectetuer adipiscing elit. You can <a href="http://themes.3rdwavemedia.com/website-templates/orbit-free-resume-cv-template-for-developers/"target="_blank">download this free resume/CV template here</a>.Aenean commodo ligula eget dolor aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis, ultricies nec, pellentesque eu.',
    'career' => [
        [
            'post' => 'Lead Developer',
            'place' => 'Startup Hubs, San Francisco',
            'duty' => 'Describe your role here lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis, ultricies nec, pellentesque eu, pretium quis, sem. Nulla consequat massa quis enim. Donec pede justo.',
            'yearStart' => 2016,
            'yearEnd' => 2017
        ],
        [
            'post' => 'Senior Software Engineer',
            'place' => 'Google, London',
            'duty' => 'Describe your role here lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis, ultricies nec, pellentesque eu, pretium quis, sem.',
            'yearStart' => 2018,
            'yearEnd' => 2020
        ],
        [
            'post' => 'UI Developer',
            'place' => 'Amazon, London',
            'duty' => 'Describe your role here lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis, ultricies nec, pellentesque eu, pretium quis, sem.',
            'yearStart' => 2020,
            'yearEnd' => 2021
        ]
    ],
    'projects' => [
        'intro' => 'You can list your side projects or open source libraries in this section. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum et ligula in nunc bibendum fringilla a eu lectus.',
        'items' => [
            [
                'title' => 'Velocity',
                'tagline' => 'A responsive website template designed to help startups promote, market and sell their products.'
            ],
            [
                'title' => 'DevStudio',
                'tagline' => 'A responsive website template designed to help web developers/designers market their services.'
            ],
            [
                'title' => 'Tempo',
                'tagline' => 'A responsive website template designed to help startups promote their products or services and to attract users &amp; investors.'
            ],
            [
                'title' => 'Atom',
                'tagline' => 'A comprehensive website template solution for startups/developers to market their mobile apps.'
            ],
            [
                'title' => 'Delta',
                'tagline' => 'A responsive Bootstrap one page theme designed to help app developers promote their mobile apps.'
            ],
        ]
    ],
    'skillset' => [
        [
            'title' => 'HTML5 &amp; CSS',
            'data-level' => '98%'
        ],
        [
            'title' => 'Angular 2 &amp; TypeScript',
            'data-level' => '80%'
        ],
        [
            'title' => 'Node &amp; Express',
            'data-level' => '75%'
        ],
        [
            'title' => 'Postgress &amp; MongoDb',
            'data-level' => '80%'
        ]
    ]
 ];
